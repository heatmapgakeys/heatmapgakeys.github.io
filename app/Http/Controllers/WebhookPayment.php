...
class WebhookPayment extends Home
{
    ...
    // ngrok ipn link works but using http protocol
    public function paypro_ipn(){
        $payment_info =  request()->all();
        if(empty($payment_info)) return response()->json(['status' => '0','message'=>'Bad request'], 200);
        $response_raw = json_encode($payment_info);
        $query_string = null;
        parse_str($payment_info['CHECKOUT_QUERY_STRING'] ?? '',$query_string);

        $paypro_whitelabel_discount_percent = $query_string['coupon-percent'] ?? 0;
        $buyer_email = $payment_info['CUSTOMER_EMAIL'] ?? "";
        $ipn_type_name = $payment_info['IPN_TYPE_NAME'] ?? ""; // SubscriptionChargeSucceed,OrderCharged
        $buyer_user_id = $parent_user_id = $package_id = null;
        if(!empty($query_string)){ // only first payment returns this info, recurring does not
            $buyer_user_id =  $query_string['user-id'] ?? null; // buyer user id
            $parent_user_id =  $query_string['seller-id'] ?? null; //seller user id
            $package_id = $query_string['package-id'] ?? null;
        }        
    
        if(empty($buyer_user_id) || empty($parent_user_id) || empty(package_id)){ // recurring payment
            $user_data = DB::table('transaction_logs')
            ->where(['buyer_email'=>$buyer_email,'payment_method'=>'PayPro'])
            ->whereNotNull('package_id')
            ->select('buyer_user_id','user_id','package_id')->orderByRaw('id DESC')->first();
            $buyer_user_id = $user_data->buyer_user_id ?? null;
            $parent_user_id = $user_data->user_id ?? null;
            $package_id = $user_data->package_id ?? null;
        }

        if(empty($buyer_user_id) || empty($parent_user_id) || empty($package_id)) 
        return response()->json(['status' => '0','message'=>'Subscription data not found.'], 200);
        

        $payment_config = $this->get_payment_config_parent($parent_user_id);
        $paypro_settings = isset($payment_config->paypro) ? json_decode($payment_config->paypro) : null;
        $test_mode_settings = isset($paypro_settings->paypro_mode) && $paypro_settings->paypro_mode=='sandbox' ? '1' : '0';
        $secret_key_settings = $paypro_settings->paypro_secret_key ?? '';
        $test_mode_ipn = $payment_info['TEST_MODE'] ?? "0";
        $hash_ipn = $payment_info['HASH'] ?? null;

        $order_id = $payment_info['ORDER_ID'] ?? "";
        $order_item_id = $payment_info['ORDER_ITEM_ID'] ?? "";
        if($test_mode_ipn!=$test_mode_settings) return response()->json(['status' => '0','message'=>'Payment modes do not match.'], 200);
        $hash_settings = $test_mode_ipn=='1' ? md5('1') : md5($order_id.$secret_key_settings);
        if($hash_settings!=$hash_ipn) return response()->json(['status' => '0','message'=>'Hashes do not match.'], 200);

        $invoice_url = $payment_info['INVOICE_LINK'] ?? null;
        $product_id = $payment_info['PRODUCT_ID'] ?? "";
        //1.Waiting,2.Canceled,3.Refunded,4.Chargeback 5.Processed
        $order_status = $payment_info['ORDER_STATUS'] ?? "";
        $order_status_id = $payment_info['ORDER_STATUS_ID'] ?? "";
        $order_referrer_url = isset($payment_info['ORDER_REFERRER_URL']) && !empty($payment_info['ORDER_REFERRER_URL']) ? get_domain_only($payment_info['ORDER_REFERRER_URL']) : "";
        $first_name = $payment_info['CUSTOMER_FIRST_NAME'] ?? "";
        $last_name = $payment_info['CUSTOMER_LAST_NAME'] ?? "";
        $country = $payment_info['CUSTOMER_COUNTRY_CODE'] ?? "";
        $paid_currency = $payment_info['VENDOR_BALANCE_CURRENCY_CODE'] ?? "";
        if(empty($paid_currency))$paid_currency = $payment_info['ORDER_CURRENCY_CODE'] ?? "USD";

        $is_whitelabel_purchase = $product_id==env('PAYPRO_WHITELABEL_PRODUCT_ID');
        if($is_whitelabel_purchase) $package_id = null;

        $transaction_id = $order_item_id;
        $subscription_status = $payment_info['SUBSCRIPTION_STATUS_ID'] ?? null;
        $subscription_initial_order_id = $payment_info['SUBSCRIPTION_INITIAL_ORDER_ID'] ?? null;
        $payment_amount = $payment_info['ORDER_ITEM_BALANCE_CURRENCY_TOTAL_AMOUNT'] ?? 0;
        if(empty($payment_amount) || $payment_amount==0) $payment_amount = $payment_info['ORDER_ITEM_TOTAL_AMOUNT'] ?? 0;
        $is_recurring = $subscription_status=='1' ? true : false;
        $payment_type = "PayPro";

        $log_insert_data['buyer_user_id'] = $buyer_user_id;
        $log_insert_data['call_time'] = date("Y-m-d H:i:s");
        $log_insert_data['api_response'] = $response_raw;
        ...
        $get_payment_validity_data = !empty($package_id) ? $this->get_payment_validity_data($buyer_user_id,$package_id) : [];

        //https://payproglobal.com/knowledge-base/developer-tools/url-parameters/
        ...
        $xsubscription_initial_order_id = $xsubscription_data->unique_id ?? null;
        $xpayment_type = $xsubscription_data->payment_type ?? null;

        if(!$is_whitelabel_purchase){
            if($is_recurring && $subscription_initial_order_id!=$xsubscription_initial_order_id && !empty($get_payment_validity_data)){ //subscription created
                $recurring_amount = $get_payment_validity_data['price'] ?? 0;
                $subscribed_period = $get_payment_validity_data['validity'] ?? 1;

                $subscription_data = [
                    'unique_id' => $subscription_initial_order_id,
            ...
            else DB::table('users')->where(['id'=>$buyer_user_id])->update(['subscription_enabled'=>'0','subscription_data'=>null]);
        }

        if(in_array($order_status_id,['3','4'])) {
            DB::table('users')->where(['id' => $buyer_user_id])->update(['status' => '0','comment'=>$order_status]);
            return response()->json(['status' => '0','message'=>'Order has been '.$order_status], 200);
        }

        if($order_status_id!='5') {
           return response()->json(['status' => '0','message'=>'Order failed to process'], 200);
        }
        ...
        if($previous_transaction_id == $transaction_id && get_domain_only(env('APP_URL'))!='heatsketch.test') return response()->json(['status' => '0','message'=>'Transaction ID duplicated'], 200);

        $package_name = $get_payment_validity_data['package_name'] ?? '';
        if($is_whitelabel_purchase) $package_name = 'White-label';
        $user_email = $get_payment_validity_data['email'] ?? '';
        if(empty($user_email)) $user_email = $buyer_email;
        $user_name = $get_payment_validity_data['name'] ?? '';
        if(empty($user_name)) $user_name = $first_name;

        if(!$is_whitelabel_purchase) $price = (float) $get_payment_validity_data['price'] ?? 0;
        else $price = (float) env('PAYPRO_WHITELABEL_PRICE',0);

        $is_agency = $get_payment_validity_data['is_agency'] ?? '0';
        $cycle_start_date = $get_payment_validity_data['cycle_start_date'] ?? null;
        $cycle_expired_date = $get_payment_validity_data['cycle_expired_date'] ?? null;
        $paid_at = date("Y-m-d H:i:s");

        if(!$is_whitelabel_purchase) $discount_data = $get_payment_validity_data['discount_data'] ?? null;
        else $discount_data = json_encode([
            'percent' => $paypro_whitelabel_discount_percent,
            'status'=> env('PAYPRO_WHITELABEL_DISCOUNT_STATUS','0')
        ]);
        ...
        if($is_whitelabel_purchase){
            unset($insert_data['cycle_start_date']);
            unset($insert_data['cycle_expired_date']);
            unset($insert_data['package_id']);

            $is_agency = null;
            $is_whitelabel = '1';
        }
        else $is_whitelabel = null;
    ...
    {
        dd($request->all());
        $response_raw = get_domain_only(env('APP_URL'))=='heatsketch.test' ? file_get_contents('https://ezsoci.com/botsailor-test-ipn/paypal-response.txt') : $request->response_raw;;
        $payment_info = json_decode($response_raw,TRUE);
        $post_api_from_ipn = $payment_info['api_key'] ?? "";
        if($post_api_from_ipn != $this->api_key && get_domain_only(env('APP_URL'))!='heatsketch.test') exit();
        ...
        $check_duplicate = DB::table("transaction_logs")->select('transaction_id')->where(['buyer_user_id'=>$buyer_user_id,'transaction_id'=>$transaction_id,'payment_method'=>$payment_type])->first();
        $previous_transaction_id = $check_duplicate->transaction_id ?? '';
        if($previous_transaction_id == $transaction_id && get_domain_only(env('APP_URL'))!='heatsketch.test') dd("Transaction ID duplicated.");
    ...
    public function fastspring_ipn()
    {
        $response_raw = file_get_contents("php://input");
        $response_raw_decoded = json_decode($response_raw,true);
        $payment_info = $response_raw_decoded['events'][0]['data'] ?? [];

        $custom_data = $payment_info['tags']['custom_data'] ?? "" ;
        $exp_custom_data = explode('-',$custom_data);
        $buyer_user_id = $exp_custom_data[0] ?? null;
        $parent_user_id = $exp_custom_data[1] ?? null;
        $package_id = $exp_custom_data[2] ?? null;
        $first_name = $payment_info['customer']['first'] ?? "" ;
        $last_name = $payment_info['customer']['last'] ?? "" ;
        $buyer_email = $payment_info['customer']['email'] ?? "" ;
        $country = $payment_info['address']['country'] ?? "" ;
        $paid_currency = $payment_info['currency'] ?? "USD" ;
        $payment_amount = $payment_info['total'] ?? 0 ;
        $transaction_id =  $payment_info['id'] ?? '' ;

        $log_insert_data['buyer_user_id'] = $buyer_user_id;
        $log_insert_data['call_time'] = date("Y-m-d H:i:s");
        $log_insert_data['api_response'] = $response_raw;
        $log_insert_data['payment_method'] = 'FastSpring';
        DB::table("payment_api_logs")->insert($log_insert_data);

        $check_duplicate = DB::table("transaction_logs")->select('transaction_id')->where(['buyer_user_id'=>$buyer_user_id,'transaction_id'=>$transaction_id,'payment_method'=>'FastSpring'])->first();
        $previous_transaction_id = $check_duplicate->transaction_id ?? '';
        if($previous_transaction_id == $transaction_id && get_domain_only(env('APP_URL'))!='heatsketch.test') dd("Transaction ID duplicated.");
        ...
        $discount_data = $get_payment_validity_data['discount_data'] ?? null;
        $format_settings = ['currency'=>$payment_config->currency ?? 'USD','decimal_point'=>$payment_config->decimal_point ?? null,'thousand_comma'=>'0','currency_position'=>$payment_config->currency_position ?? 'left'];
        $price = format_price($price,$format_settings,$discount_data,['which_price'=>'sale','display_currency'=>false,'return_raw_array'=>false]);
        ...
        $is_whitelabel = $is_agency;
        if($this->complete_payment($insert_data,$is_agency,$is_whitelabel,'FastSpring')) session(['payment_success' => '1']);
        else session(['payment_success' => '0']);
    ...
    public function mercadopago_action($package_id=0,$buyer_user_id=0,$parent_user_id=0)
    {
        ...
        if(get_domain_only(env("APP_URL")) == "heatsketch.test") {
