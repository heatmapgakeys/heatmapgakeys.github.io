...
class Member extends Home
{
    ...
    public function account_action(Request $request)
    {
        ...
        [
            ...
            'profile_pic'=>'nullable|sometimes|image|mimes:png,jpg,webp|max:200'
        ];
        ...
        if ($request->file('profile_pic')) {
            ...
            $prev_logo = storage_path("app/public/assets/profile/".$user_id);
            File::cleanDirectory($prev_logo);
            $request->file('profile_pic')->storeAs(
                'public/assets/profile/'.$user_id,
                $filename
            );
        }
        ...
        $request->session()->flash('save_user_profile', __('1'));
    ...
    public function buy_package($id=0)
    {
        ...
        if($this->is_agent && $package_data->is_agency!='1') {
        ...
        $fastspring_data = isset($payment_config->fastspring) ? json_decode($payment_config->fastspring) : [];
        $paypro_data = isset($payment_config->paypro) ? json_decode($payment_config->paypro) : [];
        ...
        $data['fastspring_status'] = $fastspring_status;
        $data['fastspring_api_src'] = $fastspring_data->fastspring_api_src ?? '';
        $data['fastspring_store_front'] = $fastspring_data->fastspring_store_front ?? '';

        $paypro_secret_key = $paypro_data->paypro_secret_key ?? '';
        $paypro_validation_key = $paypro_data->paypro_validation_key ?? '';
        $paypro_mode = $paypro_data->paypro_mode ?? 'sandbox';
        ...
        $fastspring_product_id = $product_data->fastspring->product_id ?? null;
        $fastspring_coupon = isset($product_data->fastspring->coupon) && $discount_valid ? $product_data->fastspring->coupon : '';
        $paypro_product_id = $product_data->paypro->product_id ?? null;
        $paypal_plan_id = $product_data->paypal->plan_id ?? null;
        $paypro_coupon = isset($product_data->paypro->coupon) && $discount_valid ? $product_data->paypro->coupon : null;
        $paypro_coupon = isset($product_data->paypro->coupon) && $discount_valid ? $product_data->paypro->coupon : null;
       ...
       if($paypal_status=='1')
       {
        ...
        $fastspring_button = '';
        if($fastspring_status == '1' && !empty($fastspring_product_id))
        {
            $no_payment_found_error = false;
            $fastspring_button ="
            <a href='#' class='list-group-item list-group-item-action flex-column align-items-start' data-fs-id='{$fastspring_product_id}' data-fs-coupon='{$fastspring_coupon}' id='fastspring_clone'>
                <div class='d-flex w-100 align-items-center'>
                  <small class='text-muted'><img class='rounded' width='60' height='60' src='".asset('assets/images/fastspring.png')."'></small>
                  <h5 class='mb-1'>".__('Pay with FastSpring')."</h5>
                </div>
            </a>";
        }

        $paypro_button = '';
        if($paypro_status == '1' && !empty($paypro_product_id))
        {
            $no_payment_found_error = false;
            $paypro_checkout_link = 'https://store.payproglobal.com/checkout';
            $billing_name = Auth::user()->name;
            if(str_contains($billing_name,' ')) {
                $billing_name = explode(' ',Auth::user()->name);
                $billing_last_name = array_pop($billing_name);
                $billing_first_name = implode(' ', $billing_name);
            } else{
                $billing_first_name = $billing_name;
                $billing_last_name = '';
            }
            $paypro_checkout_link_params = [
                'package-id' => $package_data->id,
                'user-id' => $this->user_id,
                'seller-id' => $this->parent_user_id,
                'currency' => $currency,
                'language' => !empty(config('app.locale')) ? strtoupper(config('app.locale')) : 'EN',
                'products[1][id]' => $paypro_product_id,
                'billing-email' => Auth::user()->email,
                'billing-first-name' => $billing_first_name,
                'billing-last-name' => $billing_last_name,
                'billing-contact-phone' => Auth::user()->mobile,
                'billing-address' => Auth::user()->address,
                'secret-key' => $paypro_secret_key,
                'coupon-percent' => $discount_data->percent ?? 0,
            ];
            if($package_data->is_whitelabel=='1' && Auth::user()->agent_has_whitelabel!='1'){
                $paypro_checkout_link_params['products[2][id]'] = env('PAYPRO_WHITELABEL_PRODUCT_ID');
            }
            if(!empty($paypro_coupon)) $paypro_checkout_link_params['coupon-code-to-add'] = $paypro_coupon;
            if($paypro_mode=='sandbox') $paypro_checkout_link_params['use-test-mode'] = 'true';
            if(!empty($paypro_template_id)) $paypro_checkout_link_params['page-template'] = $paypro_template_id;

            $paypro_checkout_link .= '?'.http_build_query($paypro_checkout_link_params);
            $paypro_button ="
            <a href='".$paypro_checkout_link."' target='_blank' class='list-group-item list-group-item-action flex-column align-items-start' id='paypro_clone'>
                <div class='d-flex w-100 align-items-center'>
                  <small class='text-muted'><img class='rounded' width='60' height='60' src='".asset('assets/images/paypro.png')."'></small>
                  <h5 class='mb-1'>".__('Pay with PayPro')."</h5>
                </div>
            </a>";
        }
        ...
        if($manual_payment_status=='1') {
            $no_payment_found_error = false;
            $manual_payment_button = '<button type="button" id="manual-payment-button" class="btn btn-outline-warning btn-lg">'.__('Manual Payment').'</button>';
        }


        $buttons_html = '<div class="row" id="payment_options">';
        if($paypro_button != '') $buttons_html .= '<div class="col-12 col-md-4 pt-3">'.$paypro_button.'</div>';
        if($fastspring_button != '') $buttons_html .= '<div class="col-12 col-md-4 pt-3">'.$fastspring_button.'</div>';
        ...
        if($manual_payment_button != '') $buttons_html .= '<div class="col-12 col-md-4 pt-3">'.$manual_payment_button.'</div>';
        $buttons_html .= '</div>';
    ...
    public function list_payment_api_log_data(Request $request)
    {
        ...
        foreach ($info as $key => $value)
        {
            ...
            $value->request_data = "<textarea class='hide-scroll' style='border:1px solid #eee;min-width:400px;min-height: 70px;'>".$value->api_response."</textarea>";
        }
    ...
    protected function get_usage_log_data(){
        ...
        $user_count = 0;
        if($this->is_agent){
            $user_count = DB::table('users')->where('parent_user_id',$this->user_id)->count();
        }
    ...
    {
        if($this->is_manager || config('settings.is_demo') == '1') abort('403');
        if(license_check_action() == 'double'){
            $rules =
                [
                    'currency' => 'required',
            ...
            if($this->is_admin) {
                $rules['fastspring_api_src'] = 'required_if:fastspring_status,1';
                $rules['fastspring_store_front'] = 'required_if:fastspring_status,1';
                $rules['paypro_secret_key'] = 'required_if:paypro_status,1';
                $rules['paypro_validation_key'] = 'required_if:paypro_status,1';
                $rules['paypro_template_id'] = 'string|nullable|sometimes';
            }
            else {
                $rules['fastspring_api_src'] = 'string|nullable|sometimes';
                $rules['fastspring_store_front'] = 'string|nullable|sometimes';
                $rules['paypro_secret_key'] = 'string|nullable|sometimes';
                $rules['paypro_validation_key'] = 'string|nullable|sometimes';
                $rules['paypro_template_id'] = 'string|nullable|sometimes';
            }
        }
        else{
        ...
        if(license_check_action()=='double' && $validate_data['paypal_status']=='1'){
        ...
        $validate_data['fastspring_status'] = isset($_POST['fastspring_status']) ? "1" : "0";
        $validate_data['paypro_status'] = isset($_POST['paypro_status']) ? "1" : "0";
        ...
        $validate_data['paypro_mode'] = isset($_POST['paypro_mode']) ? "sandbox" : "live";
        ...
        if(license_check_action() == 'double'){
            ...
            $insert_data['fastspring'] = json_encode(['fastspring_api_src'=>$validate_data['fastspring_api_src'],'fastspring_store_front'=>$validate_data['fastspring_store_front'],'fastspring_status'=>$validate_data['fastspring_status']]);
            $insert_data['paypro'] = json_encode(['paypro_secret_key'=>$validate_data['paypro_secret_key'],'paypro_validation_key'=>$validate_data['paypro_validation_key'],'paypro_mode'=>$validate_data['paypro_mode'],'paypro_template_id'=>$validate_data['paypro_template_id'],'paypro_status'=>$validate_data['paypro_status']]);
        }
    ...
    {
        if($this->is_manager || !$this->is_admin) abort('403');
    ...
    public function general_settings_action(Request $request)
    {
        ...
        if ($this->is_admin || ($this->is_agent && $this->agent_has_whitelabel)) {
            ...
            $rules['white_logo'] = 'nullable|sometimes|image|mimes:png,jpg,jpeg,webp|max:100';
            $rules['upload_bot_image'] = 'required|integer';
            $rules['upload_bot_video'] = 'required|integer';
            $rules['upload_bot_audio'] = 'required|integer';
            $rules['upload_bot_file'] = 'required|integer';
            ...
            $rules['tme_widget_id'] = 'nullable|sometimes';
            $rules['whatsapp_widget_id'] = 'nullable|sometimes';
            $rules['access_key_id'] = 'nullable|sometimes';
            $rules['secret_access_key'] = 'nullable|sometimes';
            $rules['default_region'] = 'nullable|sometimes';
            $rules['bucket'] = 'nullable|sometimes';
            $rules['endpoint'] = 'nullable|sometimes';

            $rules['ip_info_token'] = 'nullable|sometimes';
            $rules['ip2Location_api_key'] = 'nullable|sometimes';
        ...
        if ($this->is_admin || ($this->is_agent && $this->agent_has_whitelabel)) {
            ...
            if ($request->file('logo')) {
                ...
                $prev_logo = storage_path("app/public/assets/logo/".$filename);
            ...
            if ($request->file('white_logo')) {
                ...
                $extension = $request->file('white_logo')->getClientOriginalExtension();
                $filename = 'logo-white.' . $extension;
                $prev_white_logo = storage_path("app/public/assets/logo/".$filename);
                File::delete($prev_white_logo);
                $request->file('white_logo')->storeAs(
                    'public/assets/logo',
                    $filename
                );
            }
            
            $insert_data['logo'] = $request->logo_data ?? '';
            $insert_data['logo_alt'] = $request->white_logo_data ?? '';
            $insert_data['favicon'] = $request->favicon_data ?? '';
            $insert_data['app_name'] = $request->product_name;
            ...
            $upload_settings = [
                'bot' => [
                    'image' => $validate_data['upload_bot_image'],
                    'video' => $validate_data['upload_bot_video'],
                    'audio' => $validate_data['upload_bot_audio'],
                    'file' => $validate_data['upload_bot_file']
                ]
            ];
            ...
            $analytics_code_data = [
                ...
                'tme_widget_id' => $validate_data['tme_widget_id'],
                'whatsapp_widget_id' => $validate_data['whatsapp_widget_id'],
            ];
            $insert_data['analytics_code'] = json_encode($analytics_code_data);

            $aws_settings_data = [
                'access_key_id' => $validate_data['access_key_id'],
                'secret_access_key' => $validate_data['secret_access_key'],
                'default_region' => $validate_data['default_region'],
                'bucket' => $validate_data['bucket'],
                'endpoint' => $validate_data['endpoint'],
            ];
            $insert_data['aws_settings'] = json_encode($aws_settings_data);

            $google_app_settings_data = [
                'google_api_key'=>$validate_data['google_api_key']
            ];
            $get_ip_info = [
                'ip_info_token' =>$validate_data['ip_info_token'],
                'ip2Location_api_key'=>$validate_data['ip2Location_api_key']
            ];
            $social_apps_setting_data = [
                'google_app_setting'=>json_encode($google_app_settings_data),
                'get_ip_info'=>json_encode($get_ip_info)
            ];
            $insert_data['social_apps_setting'] = json_encode($social_apps_setting_data);
        ...
        // s3 settings also stored in .env file
        $path = base_path('.env');
        $initial_env = public_path('initial_env.txt');
        $test = file_get_contents($initial_env);
        if (file_exists($path))
        {
            $test = str_replace('AWS_ACCESS_KEY_ID=', 'AWS_ACCESS_KEY_ID='.$validate_data['access_key_id'], $test);
            $test = str_replace('AWS_SECRET_ACCESS_KEY=', 'AWS_SECRET_ACCESS_KEY='.$validate_data['secret_access_key'], $test);
            $test = str_replace('AWS_DEFAULT_REGION=', 'AWS_DEFAULT_REGION='.$validate_data['default_region'], $test);
            $test = str_replace('AWS_BUCKET=', 'AWS_BUCKET='.$validate_data['bucket'], $test);
            $test = str_replace('AWS_ENDPOINT=', 'AWS_ENDPOINT='.$validate_data['endpoint'], $test);
            file_put_contents($path,$test);
        }

        $query = DB::table('settings')->upsert($insert_data,['user_id'],$update_data);
        if ($this->is_agent  && $this->agent_has_whitelabel){
            ...
            $update_user = ['agent_domain'=>$agent_domain];
            DB::table('users')->where('id',$this->user_id)->update($update_user);
        }

        $request->session()->flash('save_agency_account_status', __('1'));
    ...
    public function api_settings_data(Request $request) // works for both email+sms and autoresponder
    {
        ...
        $sms_email_datatable_name = $is_sms=='0' ? 'table' : 'table3';
        $datatable_name = $is_autoresponder=='0' ? $sms_email_datatable_name : 'table2';
        $icon = $is_autoresponder=='0' ? '<i class="fas fa-edit"></i>' : '<i class="fa fa-sync"></i>';
        ...
        foreach ($info as $key => $value)
        {
            ...
            $str=$str."&nbsp;<a href='".$delete_route."' data-id='".$value->id."' data-table-name='".$datatable_name."' class='delete-row btn btn-circle btn-outline-danger'>".'<i class="fas fa-trash"></i>'."</a>";
            $value->actions = "<div style='min-width:50px'>".$str."</div>";
        }
    ...
    public function update_api_settings(Request $request) // works for both email and autoresponder
    {
        if(config('settings.is_demo') == '1') abort('403');
        $id = $request->id;
        $is_autoresponder = $request->is_autoresponder;
        $table = $is_autoresponder=='0' ? 'settings_sms_emails' : 'settings_email_autoresponders';
        ...
        foreach ($info as $key => $value)
        {
            ...
            if (! isset($value->actions)) {
                ...
                else
                {
                    ...
                    elseif ('2' == $status) $value->actions = '<a href="#" id="manual-payment-resubmit" data-id="' . $value->id . '" data-toggle="tooltip" title="' . __('You can re-submit this payment.') . '">'. __('Re-submit') .'</a>';
                }
            }
