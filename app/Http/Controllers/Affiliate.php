...
class Affiliate extends Home
{
	...
	public function new_method(Request $request)
	{
	    ...
	    $bank_acc_no = trim(strip_tags($request->input('bank_acc_no')));
	...
	public function issue_new_request(Request $request)
	{
		...
		if($submit_action == 'add') {
		    ...
		    if(DB::table('affiliate_withdrawal_requests')->insert($insert_data)) {
		        ...
		        $email_template_info =DB::table("settings_email_templates")->select('subject','message')->where('template_type','new_withdrawal_request')->first();

		        if(isset($email_template_info) && $email_template_info->subject != '' && $email_template_info->message != '')
		        {
		            // #AFFILIATOR_NAME#,#AFFILIATOR_EMAIL#,#REQUESTED_AMOUNT#
		            $subject = str_replace('#APP_NAME#',$productname,$email_template_info->subject);
		            $message = str_replace(array("#AFFILIATOR_NAME#","#AFFILIATOR_EMAIL#","#REQUESTED_AMOUNT#"),array($affiliate_name,$affiliate_email,$requested_amount),$email_template_info->message);

		        } else
		        {
		            $subject = __("Affiliate Withdrawal Request");
		            ...
		            $message .= "<ul>
		                <li>".__("Affiliator Name").": {$affiliate_name}</li>
		                <li>".__("Affiliator Email").": {$affiliate_email}</li>
		                <li>".__("Requested Amount").": {$requested_amount}</li>
		            </ul>";
