...
if ( ! function_exists('set_agency_config')){
    function set_agency_config()
    {
        ...
        return ['app_name'=>$app_name,'app_logo'=>$app_logo,'app_logo_alt'=>$app_logo_alt,'app_favicon'=>$app_favicon,'email_settings'=>$email_settings];
    }
}

if ( ! function_exists('set_agency_config_by_subscriber'))
{
    function set_agency_config_by_subscriber($subscriber_id = ''){
        if($subscriber_id=='') return false;
        $result = DB::table('telegram_bot_subscribers')->select('user_id')->where(['subscriber_id'=>$subscriber_id])->first();
        if(empty($result)) return false;
        $user_id  = $result->user_id ?? 0;
        if($user_id==0) return false;
        set_agency_config($user_id);
    }
}
...
{
    function get_agent_id($use_email=false){
        if(!$use_email) {

            if(isset($_GET['at']) && ENV('APP_URL')=='heatsketch.test') return (int) $_GET['at']; // for test purpose

            $agent_domain = get_base_url();
            $get_data = DB::table('users')->select('id')->where('agent_domain','=',$agent_domain)->first();
            return $get_data->id ?? ''; // for live server
        }
        else return isset($_GET['email']) ? $_GET['email'] : '';
    }
}
...
if ( ! function_exists('initiate_agency_config'))
{
    function initiate_agency_config($use_email=false,$ignore_logged_in=false){
        config(
            [
                'app.logo' => config('app.logo'),
                'app.logo_alt' => asset('storage/app/public/assets/logo/logo-white.png'),
                'app.favicon' => config('app.favicon')
            ]
        );
        $user_id = get_agent_id($use_email);
        if(empty($user_id)) return [];
        if(!$use_email) $agency_config = set_agency_config(null,$user_id,$ignore_logged_in);
        else $agency_config = set_agency_config($user_id,null,$ignore_logged_in);
...
if ( ! function_exists('check_is_agency_site')) {
    function check_is_agency_site(){
        return get_base_url()!=url_convert_to_domain(ENV('APP_URL')) || !empty(get_agent_id());
    }
}
...
if ( ! function_exists('get_country_iso_phone_currency_list')) {
    function get_country_iso_phone_currency_list($return = 'country') // country,currency_name,currecny_icon,phonecode
    {
        $countries = array(
            ...
            array('name' => 'Serbia', 'iso_alpha2' => 'RS', 'iso_alpha3' => 'SRB', 'iso_numeric' => '123', 'calling_code' => '381', 'currency_code' => 'RSD', 'currency_name' => 'Serbian dinar', 'currency_symbol' => 'din'),
        );
...
if ( ! function_exists('has_module_access')) {
    function has_module_access($module_id=0,$user_module_ids=[],$is_admin=false,$is_manager=false){
        ...
        if($module_id == 13)
        {
            $check_module = DB::table('modules')->select('id')->where('id',$module_id)->first();
            if(empty($check_module)) return false;
...
if(! function_exists('get_selected_sidebar')){
    ...
    {
        $map_list = [
            'subscriber-manager' => ['subscriber-manager','list-subscriber','list-label'],
            'whatsapp-subscriber-manager' => ['whatsapp-subscriber-manager','whatsapp-list-subscriber','whatsapp-list-label'],
...
if ( ! function_exists('get_public_path'))
{
    function get_public_path($param='') {
        $replace = get_domain_only(ENV('APP_URL'))=='heatsketch.test' ? 'public' : 'html';
...
if ( ! function_exists('ip_information')){
    ...
    {
        $function_list=array("get_ip_info_from_geo_plugin",'get_ip_info_from_ip2location','get_ip_from_ipinfo');
        shuffle($function_list);
        $ip_info=$function_list[0]($ip);
        if(!isset($ip_info['country']) || $ip_info['country'] != '')
            $ip_info=$function_list[1]($ip);
        if(!isset($ip_info['country']) || $ip_info['country'] != '')
            $ip_info=$function_list[2]($ip);
        return $ip_info;
    }

}

function curl_call_url($url){
...
function get_ip_info_from_geo_plugin($ip)
{
    ...
    $ip_info_json=curl_call_url($link);
...
function get_ip_info_from_ip2location($ip)
{
    ...
    if($api_key == '')
        $link="https://api.ip2location.io/?ip={$ip}";
    else
        $link="https://api.ip2location.io/?ip={$ip}&key={$api_key}";
    ...
    if(!empty($ip_info_array)){
        $ip_info['city']=$ip_info_array['city_name'] ?? ""; 
        $ip_info['country']=$ip_info_array['country_code'] ?? ""; 
        $ip_info['latitude']=$ip_info_array['latitude'] ?? ""; 
        $ip_info['longitude']=$ip_info_array['longitude'] ?? ""; 
    }
...
function get_ip_from_ipinfo($ip)
{
    ...
    if($token == '')
        ...
        $link="http://ipinfo.io/{$ip}/json?token={$token}";
...
{
    function get_table_names($user_id,$domain_code)
    {
        $response = [];
        $session_data_table_name = 'visitor_analysis_session_data_';
        $temp_session_data_table_name = 'visitor_analysis_temp_sessions_';
        $heatmap_data_table_name = 'visitor_analysis_domain_list_data_';
        $temp_heatmap_data_table_name = 'visitor_analysis_temp_heatmap_data_';

        if((int)$user_id % 2 == 0)
        {
            $session_data_table_name .= 'even_';
            $temp_session_data_table_name .= 'even_';
            $heatmap_data_table_name .= 'even_';
            $temp_heatmap_data_table_name .= 'even_';
        }
        else
        {
            $session_data_table_name .= 'odd_';
            $temp_session_data_table_name .= 'odd_';
            $heatmap_data_table_name .= 'odd_';
            $temp_heatmap_data_table_name .= 'odd_';
        }

        if((int)$domain_code % 2 == 0)
        {
            $session_data_table_name .= 'even';
            $temp_session_data_table_name .= 'even';
            $heatmap_data_table_name .= 'even';
            $temp_heatmap_data_table_name .= 'even';
        }
        else
        {
            $session_data_table_name .= 'odd';
            $temp_session_data_table_name .= 'odd';
            $heatmap_data_table_name .= 'odd';
            $temp_heatmap_data_table_name .= 'odd';
        }

        $response['sessions_table'] = $session_data_table_name;
        $response['temp_sessions_table'] = $temp_session_data_table_name;
        $response['heatmap_table'] = $heatmap_data_table_name;
        $response['temp_heatmap_table'] = $temp_heatmap_data_table_name;
...
if( ! function_exists('license_check_action') ){

    function license_check_action()
    {
		return 'double';
    }
}